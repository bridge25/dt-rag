...
async def change_password(
    ...
    try:
        ...
        user_info = await security_manager.auth_service.validate_token(credentials.credentials)
        if not user_info:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
...
async def submit_data_subject_request(
    ...
    try:
        ...
        data_request = await security_manager.compliance_manager.submit_data_subject_request(
        ...
        return {
            ...
            "response_due_date": data_request.response_due_date.isoformat() if data_request.response_due_date else None,
...
async def run_compliance_check(
    ...
    """
    try:
        compliance_report = await security_manager.compliance_manager.run_compliance_check(
            request.regulation
        )
...
async def get_security_dashboard(
    ...
    """
    try:
        dashboard_data = await security_manager.security_monitor.get_security_dashboard()
...
async def get_scan_result(
    ...
    """
    try:
        scan_result = await security_manager.vulnerability_scanner.get_scan_result(scan_id)
