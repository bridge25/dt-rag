...
class ClassifyRequest(BaseModel):
    ...
    max_labels: Optional[int] = Field(
        default=5, description="Maximum number of labels to return"
    )
    metadata: Dict[str, Any] = Field(
...
class ClassifyResponse(BaseModel):
    ...
    predictions: List[ClassificationResult] = Field(
        default_factory=list, description="Classification predictions"
    )
...
class RetrievalConfig(BaseModel):
    ...
    search_type: str = Field(
        default="hybrid", description="Search type (bm25|vector|hybrid)"
    )
    filters: Dict[str, Any] = Field(default_factory=dict, description="Search filters")
...
class FeaturesConfig(BaseModel):
    ...
    feature_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Feature parameters"
    )
