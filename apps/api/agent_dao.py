...
from apps.api.services.avatar_service import AvatarService  # @CODE:AGENT-DAO-AVATAR-002
import logging
...
class AgentDAO:
    ...
    async def create_agent(
        ...
        # @CODE:AGENT-DAO-AVATAR-002 - Pokemon Avatar System Integration
        # Calculate rarity based on taxonomy node count
        ...
        # @CODE:AGENT-DAO-AVATAR-002 - Auto-assign avatar after agent creation
        # Note: agent.agent_id is auto-generated by SQLAlchemy default
        # We need to wait for session.add() to generate the UUID
        session.add(agent)
        await session.flush()  # Flush to generate agent_id

        # Now assign avatar_url and rarity using the generated agent_id
        agent.rarity = rarity
        agent.avatar_url = AvatarService.get_default_avatar_icon(
            rarity, str(agent.agent_id)
        )
        ...
        logger.info(
            f"Agent created: {agent.agent_id} - {agent.name} "
            f"(rarity: {agent.rarity}, avatar: {agent.avatar_url})"
        )
