...
class TaxonomyNode(Base):
    ...
    canonical_path: Mapped[List[str]] = mapped_column(
...
class Embedding(Base):
    ...
    vec: Mapped[List[float]] = mapped_column(
        get_array_type(Float), nullable=False
    )  # Vector 저장
...
class CaseBank(Base):
    ...
    query_vector: Mapped[List[float]] = mapped_column(
        get_array_type(Float), nullable=False
    )
    ...
    async def _insert_default_taxonomy(session, version: int) -> None:
        ...
        for node_name, path, ver in default_nodes:
            ...
            await session.execute(
                insert_query,
                {"node_name": node_name, "canonical_path": path, "version": ver},
            )
