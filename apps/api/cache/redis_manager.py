...
class RedisManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Redis connected successfully to {self.config.host}:{self.config.port}"
            )
            return True
    ...
    async def lrem(self, key: str, count: int, value: Any) -> int:
        ...
        try:
            ...
            logger.warning(f"Redis LREM failed for key {key}: {e}")
            self.stats["operations_failed"] += 1
            return 0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (self.stats["operations_success"] / total_ops * 100) if total_ops > 0 else 0
        )
...
async def get_redis_manager() -> RedisManager:
    ...
    if _redis_manager is None:
        ...
        config = RedisConfig(
            ...
            enable_compression=os.getenv("REDIS_ENABLE_COMPRESSION", "true").lower()
            == "true",
        )
