...
class AgentTaskQueue:
    ...
    async def remove_job(self, task_id: str) -> bool:
        ...
        for priority in ["high", "medium", "low"]:
            ...
            try:
                items = await self.job_queue.redis_manager.lrange(queue_key, 0, -1)  # type: ignore[union-attr]
                ...
                for item in items:
                    try:
                        ...
                        if job_payload.get("job_id") == task_id:
                            removed_count = await self.job_queue.redis_manager.lrem(  # type: ignore[union-attr]
                                queue_key,
                                1,
                                (
