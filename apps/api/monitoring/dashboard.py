...
class MonitoringDashboard:
    ...
    async def _get_performance_summary(self) -> Dict[str, Any]:
        ...
        # 통계 계산
        latencies = [data["p95_latency"] for data in trend_data if data["p95_latency"] > 0]
        ...
        return {
            ...
            "latency_stats": {
                "avg_p95_ms": round(sum(latencies) / len(latencies), 2) if latencies else 0,
                "max_p95_ms": max(latencies) if latencies else 0,
                "min_p95_ms": min(latencies) if latencies else 0
            },
            "throughput_stats": {
                "avg_qps": round(sum(qps_values) / len(qps_values), 2) if qps_values else 0,
    ...
    async def _get_active_alerts(self) -> List[Dict[str, Any]]:
        ...
        if health.overall_status == HealthStatus.UNHEALTHY:
            alerts.append({
                ...
                "details": [comp.name for comp in health.components if comp.status == HealthStatus.UNHEALTHY]
            })
            ...
            alerts.append({
                ...
                "timestamp": health.timestamp.isoformat(),
                "details": [comp.name for comp in health.components if comp.status == HealthStatus.DEGRADED]
            })
        ...
        if current_snapshot.p95_latency > 1000:
            alerts.append({
                "level": "critical" if current_snapshot.p95_latency > 2000 else "warning",
                "type": "performance",
        ...
        if current_snapshot.total_requests > 0:
            error_rate = (current_snapshot.failed_requests / current_snapshot.total_requests) * 100
            if error_rate > 1.0:
                alerts.append({
    ...
    async def get_search_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "search_metrics": {
                "total_searches": self.metrics_collector.counters.get("search_total", 0),
                "successful_searches": self.metrics_collector.counters.get("search_success", 0),
                "failed_searches": self.metrics_collector.counters.get("search_error", 0),
                "avg_search_time_ms": self.metrics_collector.gauges.get("avg_search_time", 0)
            }
        }
    ...
    async def export_metrics_data(self, format: str = "json") -> str:
        ...
        if format.lower() == "json":
            return json.dumps(dashboard_data, indent=2, ensure_ascii=False, default=json_serializer)
        else:
    ...
    async def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        trend_data = self.metrics_collector.get_performance_trend(duration_minutes=hours * 60)
        ...
        slo_compliance = {
            ...
            "latency_compliance": sum(1 for lat in latencies if lat <= 1000) / len(latencies) * 100 if latencies else 100,
            "error_rate_target_percent": 1.0,
            "error_rate_compliance": sum(1 for err in error_rates if err <= 1.0) / len(error_rates) * 100 if error_rates else 100,
            "cache_hit_rate_target_percent": 70.0,
            "cache_hit_rate_compliance": sum(1 for data in trend_data if data["cache_hit_rate"] >= 70) / len(trend_data) * 100
        }
        ...
        return {
            ...
            "performance_summary": {
                ...
                "reliability": {
                    ...
                    "total_requests": sum(data["total_requests"] for data in trend_data),
                    "total_errors": sum(data["failed_requests"] for data in trend_data)
                }
            },
            "slo_compliance": slo_compliance,
            "recommendations": self._generate_performance_recommendations(slo_compliance, trend_data)
        }

    def _generate_performance_recommendations(self, slo_compliance: Dict[str, Any], trend_data: List[Dict[str, Any]]) -> List[str]:
        """성능 개선 권장사항 생성"""
        recommendations = []

        # 지연시간 개선
        if slo_compliance["latency_compliance"] < 95:
            recommendations.append("Consider optimizing database queries or adding more caching layers")
