...
async def _execute_optimized_search(
    request: SearchRequest, query_id: str, start_time: float
) -> SearchResponse:
    """최적화된 검색 실행"""
    try:
        # 동시성 제어
...
async def _execute_search(
    request: SearchRequest, query_id: str, start_time: float, search_type: str
) -> SearchResponse:
    """검색 실행 래퍼"""
...
async def _hybrid_engine_search(
    ...
    try:
        ...
        if cached_results:
            ...
            return _convert_to_response(cached_results, latency, query_id, "cached")
