...
class HybridScoreFusion:
    ...
    def fuse_scores(self, 
                   ...
                   vector_scores: List[float]) -> List[float]:
    ...
    def adaptive_fusion(self, 
                       ...
                       query_characteristics: Dict[str, float]) -> List[float]:
...
class ResultCache:
    ...
    def get(self, 
            query: str, 
            filters: Dict[str, Any],
            top_k: int) -> Optional[List[SearchResult]]:
        """Get cached results"""
...
class HybridSearchEngine:
    ...
    async def _perform_bm25_search(self,
        ...
        try:
            ...
            async with db_mgr.async_session() as session:
                ...
                logger.debug(f"BM25 search: {len(search_results)} results in {bm25_time:.3f}s")
    ...
    async def _perform_vector_search(self,
        ...
        try:
            ...
            async with db_mgr.async_session() as session:
                ...
                logger.debug(f"Vector search: {len(search_results)} results in {vector_time:.3f}s")
    ...
    def _fuse_results(self,
        ...
        if len(set(bm25_scores + vector_scores)) > 1:  # Check if we have varied scores
            ...
            hybrid_scores = [
                self.score_fusion.bm25_weight * bm25 + self.score_fusion.vector_weight * vector
    ...
    def _analyze_query(self, query: str) -> Dict[str, float]:
        ...
        return {
            "length": len(terms),
            "exact_terms": any(term.startswith('"') or term.endswith('"') for term in terms),
            "semantic_complexity": min(1.0, len([t for t in terms if len(t) > 6]) / max(1, len(terms))),
    ...
    async def keyword_only_search(self,
                                 query: str,
                                 top_k: int = 5,
                                 filters: Optional[Dict[str, Any]] = None) -> Tuple[List[SearchResult], SearchMetrics]:
    ...
    async def vector_only_search(self,
        ...
        try:
            ...
            results = await self._perform_vector_search(query_embedding, top_k, filters or {})
...
async def hybrid_search(query: str,
    ...
    for result in results:
        api_result = {
            ...
            "score": result.rerank_score if result.rerank_score > 0 else result.hybrid_score,
            "metadata": {
                **result.metadata,
...
async def keyword_search(query: str,
                        top_k: int = 5,
                        filters: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    return api_results, {"total_time": metrics.total_time, "results_count": len(api_results)}
