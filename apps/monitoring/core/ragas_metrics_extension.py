...
class RAGASMetricsExtension:
    ...
    def __init__(self, registry=None):
        ...
        if not self.enabled:
            logger.warning("RAGAS metrics extension disabled - Prometheus not available or no registry")
            return
    ...
    def record_evaluation_metrics(self, metrics: Dict[str, float],
        ...
        try:
            ...
            if 'faithfulness' in metrics:
                ...
                self.ragas_score_distribution.labels(metric_name='faithfulness').observe(score)

            if 'answer_relevancy' in metrics:
                score = metrics['answer_relevancy']
                self.ragas_answer_relevancy_score.set(score)
                self.ragas_score_distribution.labels(metric_name='answer_relevancy').observe(score)

            if 'context_precision' in metrics:
                score = metrics['context_precision']
                self.ragas_context_precision_score.set(score)
                self.ragas_score_distribution.labels(metric_name='context_precision').observe(score)

            if 'context_recall' in metrics:
                score = metrics['context_recall']
                self.ragas_context_recall_score.set(score)
                self.ragas_score_distribution.labels(metric_name='context_recall').observe(score)

            # Calculate and record overall score
            ragas_scores = [metrics.get(m, 0.0) for m in ['faithfulness', 'answer_relevancy', 'context_precision', 'context_recall']]
            if any(score > 0 for score in ragas_scores):
                overall_score = sum(ragas_scores) / len([s for s in ragas_scores if s > 0])
                self.ragas_overall_score.set(overall_score)

            # Record taxonomy-specific metrics
            if 'classification_accuracy' in metrics:
                self.taxonomy_classification_accuracy.set(metrics['classification_accuracy'])
    ...
    def update_quality_gates_status(self, quality_gates: Dict[str, Dict[str, float]]):
        ...
        try:
            for metric_name, gate_data in quality_gates.items():
                ...
                self.quality_gate_status.labels(metric_name=metric_name).set(status_value)

                # Set threshold
                if 'threshold' in gate_data:
                    self.quality_gate_threshold.labels(metric_name=metric_name).set(gate_data['threshold'])

                # Set gap (negative = passing, positive = failing)
                if 'gap' in gate_data:
                    self.quality_gate_gap.labels(metric_name=metric_name).set(gate_data['gap'])

            logger.debug(f"Updated quality gates status for {len(quality_gates)} metrics")

        except Exception as e:
            logger.error(f"Failed to update quality gates status: {e}")

    def record_batch_evaluation_progress(self, batch_id: str,
                                       total_queries: int,
        ...
        try:
            ...
            self.batch_evaluation_progress.labels(
                batch_id=batch_id,
                status=status
            ).set(progress)

            logger.debug(f"Updated batch {batch_id} progress: {completed_queries}/{total_queries} ({status})")
    ...
    def get_ragas_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "ragas_metrics": {
                    "faithfulness": self.ragas_faithfulness_score._value._value if hasattr(self.ragas_faithfulness_score, '_value') else 0.0,
                    "answer_relevancy": self.ragas_answer_relevancy_score._value._value if hasattr(self.ragas_answer_relevancy_score, '_value') else 0.0,
                    "context_precision": self.ragas_context_precision_score._value._value if hasattr(self.ragas_context_precision_score, '_value') else 0.0,
                    "context_recall": self.ragas_context_recall_score._value._value if hasattr(self.ragas_context_recall_score, '_value') else 0.0,
                    "overall_score": self.ragas_overall_score._value._value if hasattr(self.ragas_overall_score, '_value') else 0.0
                },
                "taxonomy_metrics": {
                    "classification_accuracy": self.taxonomy_classification_accuracy._value._value if hasattr(self.taxonomy_classification_accuracy, '_value') else 0.0,
                    "consistency_score": self.taxonomy_consistency_score._value._value if hasattr(self.taxonomy_consistency_score, '_value') else 0.0,
                    "path_precision": self.taxonomy_path_precision._value._value if hasattr(self.taxonomy_path_precision, '_value') else 0.0,
                    "hierarchical_coherence": self.hierarchical_coherence_score._value._value if hasattr(self.hierarchical_coherence_score, '_value') else 0.0
                },
                "success_rate": self.evaluation_success_rate._value._value if hasattr(self.evaluation_success_rate, '_value') else 0.0
            }
...
def extend_metrics_collector(metrics_collector):
    ...
    if not hasattr(metrics_collector, 'registry'):
        logger.warning("MetricsCollector does not have a registry - RAGAS extension disabled")
    ...
    metrics_collector.record_evaluation_metrics = ragas_extension.record_evaluation_metrics
    metrics_collector.update_quality_gates_status = ragas_extension.update_quality_gates_status
    metrics_collector.record_batch_evaluation_progress = ragas_extension.record_batch_evaluation_progress
    metrics_collector.update_evaluation_success_rate = ragas_extension.update_evaluation_success_rate
