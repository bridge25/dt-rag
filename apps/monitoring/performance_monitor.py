...
class RAGPerformanceMonitor:
    ...
    async def start_monitoring(self, session_factory):
        ...
        self._monitor_task = asyncio.create_task(
            self._monitoring_loop(session_factory)
        )
    ...
    async def _collect_db_metrics(self, session: AsyncSession, metrics: PerformanceMetrics):
        ...
        try:
            ...
            if row:
                metrics.db_query_avg_time = float(row.avg_time) / 1000  # Convert to seconds
    ...
    async def _collect_search_metrics(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            try:
                ...
                metrics.search_cache_hit_rate = engine_stats.get('cache_hit_rate', 0.0) * 100
    ...
    async def _collect_data_metrics(self, session: AsyncSession, metrics: PerformanceMetrics):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        latencies = [m.search_avg_latency for m in recent_metrics if m.search_avg_latency > 0]
        p95_latencies = [m.search_p95_latency for m in recent_metrics if m.search_p95_latency > 0]
