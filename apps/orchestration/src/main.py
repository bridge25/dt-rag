            ...
            try:
                ...
                if e.response.status_code in [429, 500, 502, 503, 504]:
                    if attempt < max_retries:
                        # 지수 백오프 + 지터
                        delay = backoff_factor * (2**attempt) + random.uniform(
                            0, jitter_max
                        )
                        logging.warning(
                            f"API 호출 실패 (시도 {attempt + 1}/{max_retries + 1}): {e.response.status_code}. {delay:.2f}초 후 재시도"
                        )
                        await asyncio.sleep(delay)
                        continue
                raise HTTPException(
                    status_code=e.response.status_code,
                    detail=f"A팀 API 호출 실패: {str(e)}",
                )
                ...
                if attempt < max_retries:
                    ...
                    logging.warning(
                        f"API 연결 오류 (시도 {attempt + 1}/{max_retries + 1}): {str(e)}. {delay:.2f}초 후 재시도"
                    )
                    await asyncio.sleep(delay)
                    continue
                raise HTTPException(
...
try:
    ...
    class RetrievalConfig(BaseModel):
        ...
        vector_topk: int = 12
        rerank: Dict = Field(
            default_factory=lambda: {"candidates": 50, "final_topk": 5}
        )
...
async def api_taxonomy_tree(version: str) -> None:
    try:
        ...
        return {
            "version": version,
            "tree": [],
            "error": f"A-team taxonomy error: {str(e)}",
        }
...
async def run_chat_pipeline(request: dict) -> None:
    ...
    """
    from langgraph_pipeline import PipelineRequest, get_pipeline
...
async def create_agent_from_category(request: FromCategoryRequest) -> None:
    ...
    features_config = FeaturesConfig(
        debate=False, hitl_below_conf=0.70, cost_guard=True
    )
...
async def suggest_cbr_cases(request: CBRSuggestRequest) -> None:
    ...
    from cbr_system import (
        CBRRecommendationEngine,
        CBRUsageLogger,
        create_query_vector_real,
    )
...
async def get_taxonomy_tree(version: str) -> None:
    ...
    try:
        ...
        # A팀 API 호출 실패 시 폴백
        return {"version": version, "tree": [], "error": f"A팀 API 호출 실패: {str(e)}"}
