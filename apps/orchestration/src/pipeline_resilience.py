...
class MemoryMonitor:
    ...
    def check_memory_health(self) -> Dict[str, Any]:
        ...
        if current_mb >= self.thresholds.critical_mb:
            ...
            warnings.append(f"메모리 사용량이 크리티컬 레벨({self.thresholds.critical_mb:.0f}MB)을 초과했습니다")
            actions_needed.extend(["즉시 가비지 컬렉션", "메모리 집약적 작업 중단"])
            
        elif current_mb >= self.thresholds.warning_mb:
            health_status = "warning"
            warnings.append(f"메모리 사용량이 경고 레벨({self.thresholds.warning_mb:.0f}MB)을 초과했습니다")
            actions_needed.append("가비지 컬렉션 실행")
            
        if current_mb >= self.thresholds.max_mb:
            health_status = "emergency"
            warnings.append(f"메모리 사용량이 최대 허용치({self.thresholds.max_mb:.0f}MB)를 초과했습니다")
        ...
        try:
            while self.monitoring_active:
                ...
                if health["status"] == "critical":
                    logger.critical(f"메모리 크리티컬: {health['usage']['current_mb']:.1f}MB")
                    await self.cleanup_memory(force=True)
                    
                elif health["status"] == "warning":
                    logger.warning(f"메모리 경고: {health['usage']['current_mb']:.1f}MB")
                    await self.cleanup_memory()
                
                elif health["status"] == "emergency":
                    logger.emergency(f"메모리 응급상황: {health['usage']['current_mb']:.1f}MB")
                    await self.cleanup_memory(force=True)
                    # 필요시 추가 응급 조치
