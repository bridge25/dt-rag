"""Upgrade documents.embedding dimension from 768 to 1536

@CODE:DATABASE-001:MIGRATION:VECTOR-1536-DOCUMENTS
@SPEC:DATABASE-001
@TEST:test_schema.py::test_documents_embedding_dimension

Revision ID: da725cdb420a
Revises: 752234dc7ed7
Create Date: 2025-10-22 21:44:31.689092
...
- Update any existing sample data (will be set to NULL)
...
revision = 'da725cdb420a'
down_revision = '752234dc7ed7'
branch_labels = None
depends_on = None
        ...
        op.execute("""
            ...
            BEGIN
                IF EXISTS (
                    ...
                    RAISE NOTICE 'Dropped old documents_embedding_idx';

                    -- Step 2: Set all embeddings to NULL (incompatible dimensions)
                    UPDATE documents SET embedding = NULL WHERE embedding IS NOT NULL;

                    RAISE NOTICE 'Set existing embeddings to NULL (no data loss - development stage)';
                    ...
                    RAISE NOTICE 'Changed documents.embedding to vector(1536)';
                    ...
                    RAISE NOTICE 'Updated column comment';
                ELSE
        ...
        op.execute("""
            ...
            BEGIN
                IF EXISTS (
                    ...
                    RAISE NOTICE 'Set embeddings to NULL for rollback';
