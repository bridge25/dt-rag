"""
Load tests for Consolidation API endpoints
...
async def run_concurrent_requests(
    ...
    async def make_request():
        ...
        while time.perf_counter() < end_time:
            ...
            await asyncio.sleep(0.2)  # Longer delay for heavier operations
...
class TestConsolidationLoad:
    ...
    # @TEST:TEST-003:LOAD-CONSOLIDATION-DRY-10USERS | SPEC: SPEC-TEST-003.md
    @pytest.mark.load
    @pytest.mark.asyncio
    async def test_consolidation_dry_run_10_users(self, async_client, sample_case_bank):
        """
        Load test: 10 concurrent users on /consolidation/dry-run

        Target:
        - P95 latency within SLA (< 2s)
        - Error rate < 0.1%
    ...
    @pytest.mark.load
    @pytest.mark.asyncio
    async def test_consolidation_dry_run_50_users(self, async_client, sample_case_bank):
        """
        Load test: 50 concurrent users on /consolidation/dry-run
    ...
    @pytest.mark.load
    @pytest.mark.asyncio
    async def test_consolidation_run_10_users(self, async_client, sample_case_bank):
        """
        Load test: 10 concurrent users on /consolidation/run

        Target:
        - P95 latency within SLA (< 3s)
        - Error rate < 0.1%
