...
class TestDatabaseSchema:
    ...
    def test_confidence_constraints(self, db_connection):
        ...
        with db_connection.cursor() as cursor:
            # Setup test data
            cursor.execute(
                "INSERT INTO documents (title) VALUES ('Test') RETURNING doc_id;"
            )
            doc_id = cursor.fetchone()[0]
    ...
    def test_required_indexes_exist(self, db_connection):
        ...
        required_indexes = {
            ...
            "idx_embeddings_vec_hnsw",  # HNSW for vector similarity (upgraded from IVFFlat)
    ...
    def test_utility_functions_exist(self, db_connection):
        ...
        with db_connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT taxonomy_depth(ARRAY['AI', 'Machine Learning', 'Deep Learning']);"
            )
    ...
    def test_audit_trigger_functionality(self, db_connection):
        ...
        with db_connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM audit_log WHERE action = 'taxonomy_create';"
            )
    ...
    def test_add_to_hitl_queue_function(self, db_connection):
        ...
        with db_connection.cursor() as cursor:
            # Setup test data
            cursor.execute(
                "INSERT INTO documents (title) VALUES ('Function Test') RETURNING doc_id;"
            )
            doc_id = cursor.fetchone()[0]
