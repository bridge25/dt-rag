...
class TestAgentBackgroundTasksMigration:
    ...
    @pytest.mark.asyncio
    async def test_migration_0012_creates_background_tasks_table(
        self,
        alembic_dir
    ):
        """
        ...
        async with async_session() as session:
            ...
            assert column_dict['webhook_retry_count'][3] is not None, (
                "webhook_retry_count should have DEFAULT 0"
            )
            assert column_dict['cancellation_requested'][3] is not None, (
                "cancellation_requested should have DEFAULT FALSE"
            )
            assert column_dict['progress_percentage'][3] is not None, (
                "progress_percentage should have DEFAULT 0.0"
            )

    @pytest.mark.asyncio
    async def test_migration_0012_creates_coverage_history_table(
        self,
        alembic_dir
    ):
        """
        ...
        async with async_session() as session:
            ...
            assert column_dict['overall_coverage'][2] == 'NO', "overall_coverage should be NOT NULL"
            assert column_dict['total_documents'][2] == 'NO', "total_documents should be NOT NULL"
            assert column_dict['total_chunks'][2] == 'NO', "total_chunks should be NOT NULL"
            assert column_dict['version'][2] == 'NO', "version should be NOT NULL"

            # Verify default values
            assert 'gen_random_uuid()' in str(column_dict['history_id'][3]), (
                "history_id should have gen_random_uuid() default"
            )
            assert '1.0.0' in str(column_dict['version'][3]), (
                "version should have DEFAULT '1.0.0'"
            )

    @pytest.mark.asyncio
    async def test_background_tasks_indexes(
        self,
        alembic_dir
    ):
        """
        ...
        async with async_session() as session:
            ...
            if idx_created_at:
                assert 'DESC' in idx_created_at[1], (
                    "idx_background_tasks_created_at should be DESC ordered"
                )

    @pytest.mark.asyncio
    async def test_coverage_history_indexes(
        self,
        alembic_dir
    ):
        """
        ...
        async with async_session() as session:
            ...
            if idx_timestamp:
                assert 'DESC' in idx_timestamp[1], (
                    "idx_coverage_history_timestamp should be DESC ordered"
                )

    @pytest.mark.asyncio
    async def test_background_tasks_foreign_keys(
        self,
        alembic_dir
    ):
        """
        ...
        async with async_session() as session:
            ...
            assert fk is not None, "Foreign key on background_tasks.agent_id does not exist"
    ...
    @pytest.mark.asyncio
    async def test_coverage_history_constraints(
        self,
        alembic_dir
    ):
        """
    ...
    async def test_migration_roundtrip(
        ...
        async with async_session() as session:
            ...
            assert 'background_tasks' in tables, "background_tasks should exist after upgrade"
            assert 'coverage_history' in tables, "coverage_history should exist after upgrade"
        ...
        async with async_session() as session:
            ...
            assert 'background_tasks' not in tables_after_downgrade, (
                "background_tasks should be removed after downgrade"
            )
            assert 'coverage_history' not in tables_after_downgrade, (
        ...
        async with async_session() as session:
            ...
            assert 'background_tasks' in tables_after_reupgrade, (
                "background_tasks should be recreated after re-upgrade"
            )
            assert 'coverage_history' in tables_after_reupgrade, (
